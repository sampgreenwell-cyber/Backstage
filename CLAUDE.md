...
**Deployment (`infra/terraform/azure-container-apps/`)**: Terraform module provisioning the Azure Container Apps infrastructure (resource group, environment, the Container App resource itself). State is remote — an azurerm backend storage account (`backstagetf`/`funjibly-tfstate-rg`, Azure AD auth only, no shared key), separate from `rg-backstage` on purpose so state outlives the infra it describes. The module deliberately does **not** manage `DATABASE_URL`/`BACKEND_AUTH_SECRET` (set manually in Azure) or the running `image` — read that directory's README before changing anything touching the Container App's `secret`/`env` lists; Azure treats both as full-replacement lists, and a naive Terraform change can silently delete a manually-added secret.

**CI/CD (`.github/workflows/deploy.yml`)**: on every push/PR to `main` it builds the image and pushes to GHCR (`ghcr.io/<owner>/backstage`) tagged with the short git SHA, then runs `terraform plan -lock=false` for visibility only (the CI service principal has read-only access to the state storage). On push to `main`, a separate `deploy` job then runs `az containerapp update --image ...` pointing at that exact SHA tag — this, not Terraform, is what actually deploys each push. `terraform apply` never runs in CI — the service principal's RBAC role on state storage is Reader-only, an enforced boundary rather than just an omitted step — and stays a manual step for infra-only changes. This split exists because Container Apps only creates a new revision when the image *reference* string changes — pushing to a static `:latest` tag alone never triggers a redeploy, however many times you push to it.
